    ...
    public class DepartmentService : IDepartmentService
    {
        ...
        public async Task<ApiResponse<bool>> DeleteAsync(Guid id)
        {
            try
            {
                ...
                var userDepartments = await _unitOfWork.UserDepartments.FindAsync(ud => ud.DepartmentId == id);
                if (userDepartments.Any())
                {
                    return ApiResponse<bool>.Fail(false, "Không thể xóa phòng ban vì còn có người dùng thuộc phòng ban này");
                }
